...
public class O2OUniDireactionalMappingTests {
	...
	void prepareData() {
		...
		IntStream.rangeClosed(1, 7).forEachOrdered(seq -> {
			...
			try { 
				...
				Locker1 foundLocker = 
					this.em.<Locker1>find(Locker1.class, Long.valueOf(seq));
...
//	@Disabled
	...
	@Test
//	@RepeatedTest(1)
	@DisplayName("2. testO2OUniObjectGraphTraverseFromStudentToLocker")
	@Timeout(value=1L, unit = TimeUnit.MINUTES)
	void testO2OUniObjectGraphTraverseFromStudentToLocker() {
		log.trace("testO2OUniObjectGraphTraverseFromStudentToLocker() invoked.");
		
		// 모든 학생들을 조회해서, 각 학생의 학번과 사물함 번호를 출력함으로써,
		// JPA를 이용한 1:1, 단방향 연관관계대로 조회가 가능한지를 테스트 해보자!
		IntStream.rangeClosed(1, 7).forEachOrdered(seq -> {
			Student1 foundStudent = 
				this.em.<Student1>find(Student1.class, Long.valueOf(seq));
			
			assertNotNull(foundStudent);
			log.info("\t+ Student: id({}), locker({})", 
					foundStudent.getId(), foundStudent.getLocker().getId());
		}); // forEachOrdered
	} // testO2OUniObjectGraphTraverseFromStudentToLocker
